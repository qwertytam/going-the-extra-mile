...
function getRoutes (start, end, points, next) {
  var routeLineColour = cmap[Math.round(cmap.length * (subsetIdx + 1) / optRoute.length)]
  ...
  dirService.route(request, function (response, status) {
    if (status === google.maps.DirectionsStatus.OK) {
      ...
      const strDone = '#'
      const strToGo = '_'
      var strStatus = '[' + strDone.repeat(pctDoneScale) + strToGo.repeat(pctInterval - pctDoneScale) + '] ' + Math.round(pctDone * 100) / 100 + '%'
...
function displayRoutes () {
  ...
  var totalDistKm = Math.round(totalDist / 1000) // Convert metres to kilometres
...
function displayMarkers () {
  ...
  for (var i = 0; i < markerLocations.length; i++) {
    if (i === 0) {
      ...
      pinColour = cmap[Math.round(cmap.length * (i + 1) / markerLocations.length)]
    }
